...
farcompilestrings --symbols=lex.txt -keep_symbols=1 $train_data > text.far
ngramcount --order="$ngrams" text.far > text.cnts
ngrammake --method="$method" text.cnts > text.lm
...
farcompilestrings --symbols=test.txt -keep_symbols=1 $test_data > test.far
ngramperplexity text.lm test.far

# Clean the directory from the generated files
rm lex.txt text.far text.cnts text.lm test.txt test.far
